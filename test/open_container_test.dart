...
import 'package:open_container/src/open_container_route.dart';
...
void main() {
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      // Open the container.
      final context = tester.element(find.byType(OpenContainer));
      Navigator.push<void>(
        context,
  ...
  testWidgets('closes to the right location when src position has changed',
    ...
    final openContext = tester.element(find.text('Open'));
  ...
  testWidgets('src changes size while open', (WidgetTester tester) async {
    ...
    final Size originalClosedRect = tester.getSize(find
        .ancestor(
          of: find.text('Closed'),
          matching: find.byType(Material),
        )
        .first);
    expect(originalClosedRect, const Size(100, 100));
  ...
  testWidgets('builder has anti-alias clip by default',
      (WidgetTester tester) async {
  ...
  testWidgets('builder has no clip', (WidgetTester tester) async {
    final GlobalKey closedBuilderKey = GlobalKey();
    final Widget openContainer = OpenContainer(
      tag: _tag,
      builder: (BuildContext context) {
  ...
  testWidgets('Verify that "rootNavigator: false" uses the correct navigator',
      (WidgetTester tester) async {
    ...
    expect(find.descendant(of: find.byKey(appKey), matching: find.text('Open')),
        findsOneWidget);

    expect(
        find.descendant(
            of: find.byKey(nestedNavigatorKey), matching: find.text('Open')),
        findsOneWidget);
  });

  testWidgets('Verify that "rootNavigator: true" uses the correct navigator',
      (WidgetTester tester) async {
    const Key appKey = Key('App');
    const Key nestedNavigatorKey = Key('Nested Navigator');
  ...
  testWidgets('Verify correct opened size  when "rootNavigator: false"',
      (WidgetTester tester) async {
    const Key appKey = Key('App');
    const Key nestedNavigatorKey = Key('Nested Navigator');
    ...
    expect(tester.getSize(find.text('Open')),
        equals(tester.getSize(find.byKey(nestedNavigatorKey))));
  });

  testWidgets('Verify correct opened size  when "rootNavigator: true"',
      (WidgetTester tester) async {
    const Key appKey = Key('App');
    const Key nestedNavigatorKey = Key('Nested Navigator');
